...
class SlutPriserScraper:
        ...
        for page in range(start_page,num_of_pages+start_page):
            ...
            for property_row in soup.findAll('div', attrs = {'class':'sold-property-listing'}):
                ...
                try:
                    ...
                    location_div = property_row.find('div', attrs = {'class':'sold-property-listing__location'})
                    street_address = location_div.h2.findAll('span', attrs= {'class':'item-result-meta-attribute-is-bold item-link'})[0].text
                    location = location_div.div.text.strip() 
                    region = location.split(',')[-1].replace('\n','').replace('\t','').replace('Bostadsrättslägenhet','').replace('\xa0','').strip().replace('Bostadsrätt','').replace(' ','')
                    ...
                    if not normalized:
                        location = location.split(',')[0].replace('\n','').replace('\t','').replace('Bostadsrättslägenhet','').replace('\xa0','').strip().replace('Bostadsrätt','').replace('Andelibostadsförening','').replace(' ','').split('/')[0].split('-')[0].split('\\')[0]
                    ...
                    if use_google_maps_api:
                        dist_to_central_st = SlutPriserScraper._calculate_distance_to_central_station(street_address)
                        listing['dist_to_central_st'] = dist_to_central_st

                    if not region or not location:
                        print("Skipping property as region or location parameter not found")
                        continue

                    size_div = property_row.find('div', attrs={'class':'sold-property-listing__size'})
                    size_and_rooms = size_div.div.find('div', attrs={'class':'sold-property-listing__subheading'}).text
                    ...
                    fee = size_div.div.find('div', attrs={'class':'sold-property-listing__fee'}).text
                    fee = fee.replace('\n','').replace('\t','').replace(' ','').split('kr')[0].replace('\xa0','')
                    listing['fee'] = fee
                    ...
                    final_price_div = property_row.find('div', attrs={'class':'sold-property-listing__price'})
                    final_price = final_price_div.div.span.text
                    final_price = final_price.replace('\n','').replace('\t','').replace(' ','').split('kr')[0].split('Slutpris')[-1].replace('\xa0','')
                    ...
                    listing_soup = BeautifulSoup(listing_webpage.text, 'html.parser')

                    price_stats = listing_soup.find('dl', attrs={'class':'sold-property__price-stats'}) 
                    ...
                    initial_price = initial_price.replace('\n','').replace('\t','').replace(' ','').replace('\xa0','').replace('kr','')
                    
                    if not initial_price:
                        print("Skipping property as initial_price parameter not found")
                        continue
                    
                    listing['initial_price'] = initial_price
                    
                    attributes = listing_soup.find('dl', attrs={'class':'sold-property__attributes'}) 

                    for dt, dd in zip(attributes.findAll('dt'),attributes.findAll('dd')):
                        if "Bygg" in dt.text:
                            year_built = dd.text.split('-')[0]
                        elif "Drift" in dt.text:
                            mgmt_cost = dd.text.split('kr')[0].replace(' ','').replace('\xa0','')
                        
                    if not year_built or not mgmt_cost:
                        print("Skipping property as year_built or mgmt_cost parameter not found")
                        continue
    ...
    def _calculate_distance_to_central_station(street_address):
        ...
        URL = "https://maps.googleapis.com/maps/api/distancematrix/json?origins=%s+Stockholm&destinations=T-Centralen+Stockholm&mode=transit&key=%s" % (origin, API_KEY)
        response = get(URL)
